	...
	try {
		...
		do {
			...
			if ($ContinueMPITest) {
				...
				if (-not $CurrentTestData.TestParameters.param.Contains('endure_sku=yes')) {
				...
				if ($CurrentTestData.TestParameters.param.Contains('short_testpass=no')) {
					...
					Write-LogInfo "Short test pass chosen. Only running MPI pingpong"
				}
