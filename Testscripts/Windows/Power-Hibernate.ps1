...
"@
		...
		if([int]$env_setup -ge 1) {
			...
			Run-LinuxCmd -ip $AllVMData.PublicIP -port $AllVMData.SSHPort -username $user -password $password -command "/home/$user/hibernation-setup-tool.sh" -runAsSudo -ignoreLinuxExitCode:$true | Out-Null
			Write-LogInfo "Executed hibernation-setup-tool script inside VM"
			...
			if ($state -eq "TestCompleted") {
				...
				Write-LogErr "hibernation-setup-tool.sh didn't finish successfully!"
				$resultArr = $resultFail
				$currentTestResult.TestResult = Get-FinalResultHeader -resultarr $resultArr
				return $currentTestResult.TestResult
			} elseif ($state -eq "TestAborted") {
				Write-LogInfo "hibernation-setup-tool.sh finished with Aborted state!"
				$resultArr = $resultAborted
				$currentTestResult.TestResult = Get-FinalResultHeader -resultarr $resultArr
				return $currentTestResult.TestResult
			} else {
				Write-LogInfo "hibernation-setup-tool.sh is still running in the VM!"
			}
		}
