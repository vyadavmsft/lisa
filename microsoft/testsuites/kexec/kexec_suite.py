...
class KexecSuite(TestSuite):
    ...
    def _trigger_kexec_reboot(
        ...
        if use_systemctl:
            ...
            if result.exit_code not in (0, None):
                ...
                log.debug(
                    f"systemctl kexec returned exit code {result.exit_code}, "
    ...
    def _wait_for_reconnect(
        ...
        while (timer.elapsed(False) < self.RECONNECT_TIMEOUT) or tried_times < 1:
            ...
            log.debug(f"reconnect attempt, current boot time: {current_boot_time}")
    ...
    def _validate_kexec_reboot(
        ...
        Checks:
        - Cmdline nonce present (primary proof of kexec boot)
